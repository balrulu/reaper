...
3. Paste the following repository URL:
   ...
   ```text
   https://github.com/balrulu/reaper/raw/main/index.xml
   ```
...
## Reaper Script & FX BLT SERIES 使用許諾など

Reaper Script & FX BLT SERIES（以下「本ソフトウェア」）は、個人利用・商用利用を問わず、無料で利用できるフリーソフトウェア（REAPER上での動作を前提）です。

本ソフトウェアは現状のまま提供され、その動作や特定の目的への適合性を保証するものではありません。
あくまで、Codexの手を借りつつ個人用途で制作したものとなり、不具合報告やご意見は参考にさせていただければとは思いますが、個別のサポートや動作保証は行っておりませんのでご了承ください。

本ソフトウェアの使用によって生じたデータの損失、動作不良、その他の損害について、作者（ばルるる）は法令上認められる範囲で責任を負いません。重要なプロジェクトやデータについては、必要に応じてバックアップを取ったうえでご利用ください。
