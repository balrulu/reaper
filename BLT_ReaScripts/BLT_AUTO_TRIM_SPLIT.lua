-- @description Auto Trim / Split - waveform preview, Peak and LUFS-M (JP/EN GUI)
-- @version 0.5.0
-- @about
--   Standalone native-gfx ReaScript. js_ReaScriptAPI required for the custom title bar.
--   Detection uses active-take audio BEFORE take/track FX.
--   Take-accessor time is item-relative; edit time is project-absolute.
--   48 kHz analysis, 1 ms peak bins, K-weighted 400 ms LUFS-M for OPEN and 100 ms FAST loudness for CLOSE.
--   LUFS-M: mono/stereo; Peak: up to 64 source channels.
--   All audio reads run in the main defer callback, never a coroutine.
--   EDGE MODE retains one span between the first and last accepted regions.
...
local function draw()
 ...
 BLT.viewport(scale,gfx.ext_retina or 1);  mouse_x,mouse_y=(gfx.mouse_x-ox)/scale,(gfx.mouse_y-oy)/scale
  ...
  BLT.footer(message~='' and message or (not A.ready and '音声アイテムを選択してください。' or ''),A.bad,W,H+22,'0.5.0')
  custom_titlebar()
end
